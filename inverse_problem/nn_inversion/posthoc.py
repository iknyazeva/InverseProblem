...
def plot_analysis_hist2d_unc(refer, predicted_mu, predicted_sigma, names, index=0, mask=None, title=None, bins=100,
    ...
    ylabels = [r'$(x_{true} - x_{pred})/ \sigma_{pred}$', r'$x_{pred}$', r'$\sigma_{pred}$', r'$\sigma_{pred}$',
               r'$(x_{pred} - x_{true})/x_{true}$']
    param_labels = ['Gauss', 'Degree', 'Degree', 'm$\AA$', 'Dopplerwidths', 'Relative units', 'Normalized intensity',
                    'Normalized intensity', 'kmps', 'Normalized intensity', 'kmps']
    font = 19
...
def plot_fitting_curves_unc(refer, predicted_mu, predicted_sigma, names, mask=None, title=None, save_path=None):
    ...
    for i, ax in enumerate(axs.flat[:-1]):
        ...
        ymin, ymax = max(-30, np.percentile(a[skipping:] - b[skipping:], 0.01)), min(30, np.percentile(
            a[skipping:] + b[skipping:], 99.9))
...
def plot_fitting_curves_unc_double(refer, predicted_mu, predicted_sigma, refer2, predicted_mu2, predicted_sigma2,
                                   names=None, mask=None, title=None, save_path=None):
    """
        draws fitting curves a, b: each point of a parameter is fitted by the N(a, b)
    """
    if mask is not None:
...
def plot_params2(data, names, title=None, save_path=None, color_map='gray', inverse=False):
    ...
    if inverse:
        x2_limits = [0] * 11
        x1_limits = [1] * 11
    bar_labels = ['Gauss', 'Degree', 'Degree', 'm$\AA$', 'Dopplerwidths', 'Normalized intensity',
                  'Normalized intensity', 'Normalized intensity', 'kmps', 'Normalized intensity', 'kmps']
    for i, ax in enumerate(axs.flat[:-1]):
        x_min, x_max = np.min(data[:, :, i]), np.max(data[:, :, i])
        ax.set_title(names[i], weight='bold', fontsize=14)
        params = ax.imshow(data[:, :, i], cmap=truncate_colormap(color_map, x1_limits[i], x2_limits[i]), vmin=x_min,
...
def plot_params3(data1, data2, data3, names, title=None, save_path=None, color_map='gray', inverse=False):
    ...
    starting_point = 0.08
    font = 24
    fig.text(x_shift, starting_point + y_shift * 5, '$x_{true}$', va='center', rotation='vertical', fontsize=font,
             weight='bold')
    fig.text(x_shift, starting_point + y_shift * 4, '$x_{pred}$', va='center', rotation='vertical', fontsize=font,
             weight='bold')
    fig.text(x_shift, starting_point + y_shift * 3, r'$\sigma_{pred}$', va='center', rotation='vertical', fontsize=font,
             weight='bold')
    fig.text(x_shift, starting_point + y_shift * 2, '$x_{true}$', va='center', rotation='vertical', fontsize=font,
             weight='bold')
    fig.text(x_shift, starting_point + y_shift, '$x_{pred}$', va='center', rotation='vertical', fontsize=font,
             weight='bold')
    fig.text(x_shift, starting_point, r'$\sigma_{pred}$', va='center', rotation='vertical', fontsize=font,
             weight='bold')
    fig.set_facecolor('xkcd:white')
    axs[-1][-1].axis("off")
    axs[-2][-1].axis("off")
