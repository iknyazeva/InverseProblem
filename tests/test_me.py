...
class TestHinodeME:
    ...
    def test_compute_spectrum_zero(self):
        ...
        assert isinstance(spectrum, np.ndarray)
        assert spectrum.shape == (1, 56, 4)
        assert expected_I == pytest.approx(spectrum[0][:5, 0], rel=1e-6)
        assert expected_V == pytest.approx(spectrum[0][:5, 1], rel=1e-6)

        spectrum_ff = obj.compute_spectrum(with_ff=True, with_noise=False)
        assert isinstance(spectrum, np.ndarray)
        assert spectrum_ff.shape == (1, 56, 4)
        assert expected_I == pytest.approx(spectrum_ff[0][:5, 0], rel=1e-6)
        assert expected_V == pytest.approx(spectrum_ff[0][:5, 1], rel=1e-6)
    ...
    def test_compute_spectrum_1250(self):
        ...
        assert isinstance(spectrum, np.ndarray)
        assert spectrum.shape == (1, 56, 4)
        assert expected_I == pytest.approx(spectrum[0][:5, 0], rel=1e-4)
        assert expected_V == pytest.approx(spectrum[0][:5, 1], rel=1e-4)
    ...
    def test_compute_spectrum_ff(self):
        ...
        assert spectrum.shape == (1, 56, 4)
        assert expected_I == pytest.approx(spectrum[0][:5, 0], rel=1e-4)
        assert expected_V == pytest.approx(spectrum[0][:5, 1], rel=1e-3)
        assert True
